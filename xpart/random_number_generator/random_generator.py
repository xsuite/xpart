...
class RandomGenerator(xt.BeamElement):
    _xofields = {
...
#         context.kernels.set_rutherford(ran=self, z=material.Z, emr=material.nuclear_radius, upper_val=material.hcut)
    ...
    def sample(self, n_samples=1000, n_seeds=1000, distribution='uniform', particles=None):
        ...
        self._distribution = self.distributions[distribution]
        ...
        if n_seeds > len(self._samples):
            raise ValueError("More seeds requested than capacity in RandomGenerator._samples.")
        if n_samples*n_seeds > len(self._samples):
            # Todo: resize _samples
            print("Not enough capacity in RandomGenerator._samples to allow for requested number of seeds and samples. "
