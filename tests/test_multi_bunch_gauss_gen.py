...
def test_gaussian_bunch_generation(test_context):
    ...
    filling_scheme_array = np.zeros(int(np.amin(h_list)/bunch_spacing_in_buckets))
    # build a dummy filling scheme
    n_bunches_tot = 10
    filling_scheme_array[0:int(n_bunches_tot/2)] = 1
    filling_scheme_array[n_bunches_tot:int(3*n_bunches_tot/2)] = 1
    ...
    part = xp.generate_matched_gaussian_beam(
             ...
             num_particles=n_part_per_bunch, total_intensity_particles=bunch_intensity,
    ...
    # CHECKS
    for i_bunch, bunch_id in enumerate(filling_scheme.get_bunches()):
        y_rms = np.std(test_context.nparray_from_context_array(part.y[i_bunch*n_part_per_bunch:
                                                                      (i_bunch+1)*n_part_per_bunch]))
        x_rms = np.std(test_context.nparray_from_context_array(part.x[i_bunch*n_part_per_bunch:
                                                                      (i_bunch+1)*n_part_per_bunch]))
        delta_rms = np.std(test_context.nparray_from_context_array(part.delta[i_bunch*n_part_per_bunch:
                                                                              (i_bunch+1)*n_part_per_bunch]))
        zeta_rms = np.std(test_context.nparray_from_context_array(part.zeta[i_bunch*n_part_per_bunch:
                                                                            (i_bunch+1)*n_part_per_bunch]))
    ...
    part = xp.generate_matched_gaussian_beam(
             filling_scheme=filling_scheme, engine='pyheadtail',
             num_particles=n_part_per_bunch, total_intensity_particles=bunch_intensity,
             nemitt_x=nemitt_x, nemitt_y=nemitt_y, sigma_z=sigma_z,
             line=line)
